...
<?php echo form_open('/admin/memo_firstname?n=' . $item['registration_no'], 'id="memoForm" name="memoForm"');
?>
<div class="flex flex-col items-center justify-center w-full h-full">
    <p>변경하실 first name을 입력해주세요.</p>
    <input id="first_name" name="first_name" class="w-10/12 h-4/6 border p-5"
...
async function saveMemo() {
    const memoValue = first_name.value;
    const url = form.action;
    const formData = new FormData(form);
    const registration_no = window.location.search.split("=")[1];

    if (memoValue === "") {
        formData.set('first_name', null); // 메모 필드의 값을 공백으로 설정
    } else {
        formData.set('first_name', memoValue);
    }
    try {
